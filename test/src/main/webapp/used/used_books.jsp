...
<body>
	...
	<main>
		<div class="container">
					...
					<li class="category-item">
						...
						<ul id="foreign-books" class="subcategory-list">
							<li class="subcategory-item"><a
								href="/test/used/usedbooklist/UsedBookList.jsp?subcategory=ELT/사전">ELT/사전</a>
							</li>
							<li class="subcategory-item"><a
								href="/test/used/usedbooklist/UsedBookList.jsp?subcategory=유아/어린이">유아/어린이</a></li>
						</ul>
					</li>
					<li class="category-item">
						...
						<ul id="domestic-books" class="subcategory-list">
							<li class="subcategory-item"><a
								href="/test//used/usedbooklist/UsedBookList.jsp?subcategory=자기계발">자기계발</a></li>
							<li class="subcategory-item"><a
								href="/test/used/usedbooklist/UsedBookList.jsp?subcategory=예술">예술</a></li>
							<li class="subcategory-item"><a
								href="/test/used/usedbooklist/UsedBookList.jsp?subcategory=자기계발">자기계발</a></li>
						</ul>
					</li>
			...
			<section class="used-book-upload">
				...
				<form id="registerBookForm" action="registerBookResult.jsp"
					method="POST">
						...
						<div class="book-details">
							<div class="form-row">
								<label for="book-title">도서명:</label> <input type="text"
									id="book-title" name="title" maxlength="255" readonly>
							</div>
							<div class="form-row">
								<label for="book-author">저자:</label> <input type="text"
									id="book-author" name="author" maxlength="100" readonly>
							</div>
							<div class="form-row">
								<label for="book-publisher">출판사:</label> <input type="text"
									id="book-publisher" name="publisher" maxlength="100">
							</div>
							<div class="form-row">
							...
							<div class="form-row">
								<label for="book-condition">도서 상태:</label> <select
									id="book-condition" name="status" required>
							...
							<div class="form-row">
								<label for="book-category">카테고리:</label> <input type="text"
									id="book-category" name="category" maxlength="50">
							</div>
							<div class="form-row">
								<label for="book-stock">재고:</label> <input type="number"
									id="book-stock" name="stock" min="0" required>
							</div>
							<div class="form-row">
								<label for="book-images">사진 업로드:</label> <input type="file"
									id="book-images" name="image_path" multiple accept="image/*">
							</div>
							<!-- seller_id를 입력받는 필드를 추가합니다 -->
							<input type="hidden" id="seller-id" name="seller_id"
								value="${sessionScope.sellerId}">
							<button type="button" class="submit-button" id="sellerSubmitBtn">판매
								등록</button>
						</div>
					</div>
				</form>
			...
			<%
                ...
                <div class="pagination">
                    ...
                    <c:forEach var="i" begin="1" end="${allTotalPages}">
                        <a href="?allPage=${i}" class="<c:if test='${i == allCurrentPage}'>active</c:if>">${i}</a>
                    </c:forEach>
    ...
    function moveDetail(bookId){
        ...
        location.href = "/test/used/usedBooksdetail.jsp?bookId=" + bookId + "&referer=" + encodeURIComponent(referer);
    }
</script>
</body>
</html>